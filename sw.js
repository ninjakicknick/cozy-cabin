const CACHE='cozy-cabin-shell-v1';
const SHELL=['./','./index.html','./style.css?v=66','./app.js?v=66','./manifest.webmanifest','./assets/pwa-icon.svg'];
self.addEventListener('install',event=>{event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(SHELL)).then(()=>self.skipWaiting()))});
self.addEventListener('activate',event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('cozy-cabin-')&&key!==CACHE).map(key=>caches.delete(key)))).then(()=>self.clients.claim()))});
