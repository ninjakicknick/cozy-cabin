...
chords=[[48,55,59,64],[45,52,55,60],[41,48,55,57],[43,50,57,62]]
for bar in range(16):
    chord=chords[bar%4]
    start=bar*6
    for step,note in enumerate([chord[0],chord[2]+12,chord[3]+12,chord[1]+12]):
        ...
        envelope=(1-np.exp(-t*38))*np.exp(-t/(1.4 if step else 2.4))
        ...
        out[pos:pos+count]+=tone[:count]*(.17 if step else .13)
...
with wave.open('/tmp/cabin-record.wav','w') as w:
